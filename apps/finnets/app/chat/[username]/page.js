...
export default async function ChatPage({ params }) {
  ...
  const baseUrl = process.env.DATACENTER_API_URL || 'http://localhost:3000';

  try {
    const res = await fetch(`${baseUrl}/api/bank_database/getTotalBalance?username=${username}`);
    if (res.ok) privateData = await res.json();
    const ckRes = await fetch(`${baseUrl}/api/bank_database/getTotalBalance?username=${username}&type=checking`);
    if (ckRes.ok) checkingBal = await ckRes.json();
    const svRes = await fetch(`${baseUrl}/api/bank_database/getTotalBalance?username=${username}&type=savings`);
    if (svRes.ok) savingsBal = await svRes.json(); 
    const accRes = await fetch(`${baseUrl}/api/bank_database/getAccounts?username=${username}`);
  ...
  const chat_history = [[`Welcome back, ${privateData.data.first_name}!`, false]];
